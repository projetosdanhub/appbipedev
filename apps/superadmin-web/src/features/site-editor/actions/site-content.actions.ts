...
import { SiteContent } from "../types/site-content.types";
...
const DEFAULT_SITE_CONTENT: SiteContent = {
  ...
  hero: {
    ...
    primaryCtaLink: "http://127.0.0.1:3001/register",
  ...
  voiceDemo: {
    ...
    sampleText: "Olá! Seja bem-vindo à nossa empresa. Recebi sua solicitação e já preparei sua proposta em PDF com condições exclusivas. Como posso te ajudar a fechar hoje?",
  ...
  finalCta: {
    ...
    ctaButtonLink: "http://127.0.0.1:3001/register",
...
function readStoredSiteContent(): SiteContent {
  try {
    ...
    console.error("Erro ao ler site-content.json:", err);
  }
  return DEFAULT_SITE_CONTENT;
}

let activeSiteContent: SiteContent = readStoredSiteContent();

export async function getSiteContentAction(): Promise<SiteContent> {
  return activeSiteContent;
}
...
export async function saveSiteContentAction(
  ...
  try {
    activeSiteContent = JSON.parse(JSON.stringify(newContent));
    const filePath = getSiteContentFilePath();
    fs.writeFileSync(filePath, JSON.stringify(activeSiteContent, null, 2), "utf-8");
    revalidatePath("/site-editor");
    revalidatePath("/landing");
    revalidatePath("/");
    return {
      success: true,
      message: "Conteúdo do site publicado com sucesso!",
    };
  } catch (error) {
    return {
      success: false,
      message: `Erro ao salvar conteúdo: ${(error as Error).message}`,
    };
  }
}

export async function resetSiteContentAction(): Promise<{ success: boolean; content: SiteContent; message: string }> {
  activeSiteContent = JSON.parse(JSON.stringify(DEFAULT_SITE_CONTENT));
  try {
    const filePath = getSiteContentFilePath();
    fs.writeFileSync(filePath, JSON.stringify(activeSiteContent, null, 2), "utf-8");
  } catch (e) {
    console.error("Erro ao resetar fixture:", e);
  }
  revalidatePath("/site-editor");
  revalidatePath("/landing");
  revalidatePath("/");
  return {
    ...
    message: "Conteúdo restaurado para o padrão de fábrica!",
  };
}
