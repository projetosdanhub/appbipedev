...
test.describe.serial("Autenticação Completa (AUTH)", () => {
  ...
  test("Deve realizar login com o usuário criado", async ({ page }) => {
    ...
    await page
      .getByRole("button", { name: "Logar com E-mail e Senha" })
      .click();
  ...
  test("Deve solicitar recuperação de senha e ir para verificação", async ({
    page,
  }) => {
  ...
  test("Deve bloquear reset de senha com as senhas não coincidindo", async ({
    page,
  }) => {
